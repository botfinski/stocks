...
const App = () => {
  ...
  const handleSearch = () => {
    ...
    fetch(
      `https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${inputRef.current.value}&apikey=LDGKBPO30W0GY4VS`
    )
      .then(res => res.json())
      .then(
        result => {
  ...
  const handleSetPage = (goTo, symbol) => {
    if (goTo === "details") {
      ...
      fetch(
        `https://www.alphavantage.co/query?function=OVERVIEW&symbol=${symbol}&apikey=LDGKBPO30W0GY4VS`
      )
        .then(res => res.json())
        .then(
          result => {
